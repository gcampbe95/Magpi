...
Our next build challenge came in the assembly phase. We controlled motion along two axes with two stepper motors by using coreXY belting, which was a new idea for us. While our reference videos were helpful in the CAD stage, a lot of them proved vague when it came to the specifics of an effective belt configuration. To better understand how our mechanism worked, we decided to construct a prototype from wooden skewers, cardboard, and string. As we continued with more formal implementations of our design, it helped to have a tactile way to experiment with configurations that could be modified easily and lacked the complications associated with assembling a polished final product. Our specific confusions arose from the anchoring of the belt---while our reference videos appeared to show the belt being fixed at one end, when we tried to implement this our x motion was extremely limited---our solution ultimately involved connecting the two ends that were supposed to remain fixed and giving them freedom of movement, creating a closed loop. While our fix was somewhat effective, belt difficulties remain among the greatest hindrances to smooth pen motion. 

Limited time near the end led us to refer back to our guiding principle of simplicity: a circular note-drawing motion for the pen proved too amitious, so we cut the tip off of a crayola marker for a makeshift stamp.

**Code**:
As a conventional CoreXY system, we encountered few issues with translating motor motion to XY movement. To simplify the code we decided to create a custom Motor class, as adapting the stepper motor code from a previous assignment proved tedious. For those attempting similar programs, we recommend the creation of classes for multiple instances of objects that perform similar functions. We had a little trouble getting the frequencies collected to correspond with the right note name, especially at the extreme ends of the pitch spectrum. It took a few days of modifying our buffer size and sample rate values to get the 4th octave to correspond correctly, but the resulting program was accurate and reliable. We used dicitonaries to map the notes to their corresponding positions on the musical staff. 
